...
int WinMain(int argc, char* argv[]) {
	...
	std::vector<std::string> files = pessum::luxreader::LoadLuxListFile("resources/music/songs.lux");
	...
	for (unsigned a = 0; a < display.size(); a++) {
		...
		aequus::video::win->obj->CreateButton(display[a], "button.png", false, true, 285, 50);
	...
	while (aequus::video::AllClose() == false) {
		...
		for (unsigned a = 0; a < aequus::video::output.size(); a++) {
			if (aequus::video::output[a].windowID = 1) {
				aequus::audio::music::LoadSong(files[aequus::video::output[a].objectID]);
				aequus::audio::music::PlaySong();
				aequus::video::win->objects[aequus::video::output[a].objectID].SetColor(0.5, 1.0, 0.5, 1.0);
			}
		}
	}
