    ...
    for (unsigned a = 0; a < aequus::input::events.size(); a++) {
      if (aequus::input::events[a].type == aequus::input::EventType::KEYBOARD) {
        ...
        if (aequus::input::events[a].key == int('w') && messagebox == false) {
          ...
          newbox.NewMessageBox(
              aequus::video::MessageBox::MessageBoxFlags::ERROR,
              aequus::video::win->sdlwindow, "Message Box",
              "This is a Message Box to check if message boxes work.");
          newbox.RunMessageBox();
          pessum::logging::Log(pessum::logging::LOG_DEVELOPMENT_CHECK,
                               "Checked message box", "Core");
        }
        if (aequus::input::events[a].key == int('e') && drect == false) {
          drect = true;
          pessum::logging::Log(pessum::logging::LOG_DEVELOPMENT_CHECK,
                               "Checked draw rectangle", "Core");
        }
        if (aequus::input::events[a].key == int('r') && dpoint == false) {
          dpoint = true;
          pessum::logging::Log(pessum::logging::LOG_DEVELOPMENT_CHECK,
                               "Checked draw point", "Core");
        }
        if (aequus::input::events[a].key == int('t') && dline == false) {
          dline = true;
          pessum::logging::Log(pessum::logging::LOG_DEVELOPMENT_CHECK,
                               "Checked draw line", "Core");
        }
        if (aequus::input::events[a].key == int('y') && dfill == false) {
          dfill = true;
          pessum::logging::Log(pessum::logging::LOG_DEVELOPMENT_CHECK,
                               "Checked draw fill rectangle", "Core");
        }
        ...
        if (aequus::input::events[a].key == int('i') && otext == false) {
          ...
          aequus::video::win->obj->CreateTextObj("Hello World!!!", 40, 1, 1, 1,
                                                 1.0);
