  ...
  for (unsigned a = 0; a < rawcode.luxcodelines.size(); a++) {
    ...
    for (unsigned b = 0; b < rawcode.luxcodelines[a].size(); b++) {
      if (currentLine[b] != ' ') {
        ...
        if (readTypeTwo == true) {
          newVariable.variabledefinitiontype =
              newVariable.variabledefinitiontype + currentLine[b];
        }
        if (readName == true) {
...
pessum::luxreader::Hierarchy pessum::luxreader::LoadLuxHierarchyFile(
  ...
  for (unsigned a = 0; a < rawcode.luxcodelines.size(); a++) {
    ...
    codeLine = "";
    if (currentLine[0] == '[') {
      for (unsigned b = 1; b < currentLine.size() && currentLine[b] != ']';
           b++) {
...
void pessum::luxreader::SaveLuxDataFile(std::string filedirectory,
                                        DataFile ouputdata) {
  ...
  for (unsigned a = 0; a < ouputdata.datafilevariables.size(); a++) {
    ...
    if (ouputdata.datafilevariables[a].variabletype == "vector") {
      outputline = outputline +
                   ouputdata.datafilevariables[a].variabledefinitiontype + " ";
    }
    outputline = outputline + ouputdata.datafilevariables[a].variablename;
    if (ouputdata.datafilevariables[a].variabletype == "int") {
      outputline = outputline + " " +
                   std::to_string(ouputdata.datafilevariables[a].intvalue);
    }
    if (ouputdata.datafilevariables[a].variabletype == "string") {
      outputline =
          outputline + " " + ouputdata.datafilevariables[a].stringvalue;
    }
    if (ouputdata.datafilevariables[a].variabletype == "double") {
      outputline = outputline + " " +
                   std::to_string(ouputdata.datafilevariables[a].doublevalue);
    }
    if (ouputdata.datafilevariables[a].variabletype == "vector") {
      if (ouputdata.datafilevariables[a].variabledefinitiontype == "int") {
        for (unsigned b = 0;
             b < ouputdata.datafilevariables[a].intvectorvalues.size(); b++) {
          outputline =
              outputline + " " +
              std::to_string(ouputdata.datafilevariables[a].intvectorvalues[b]);
        }
      }
      if (ouputdata.datafilevariables[a].variabledefinitiontype == "string") {
        for (unsigned b = 0;
             b < ouputdata.datafilevariables[a].stringvectorvalues.size();
             b++) {
          outputline = outputline + " " +
                       ouputdata.datafilevariables[a].stringvectorvalues[b];
        }
      }
      if (ouputdata.datafilevariables[a].variabledefinitiontype == "double") {
        for (unsigned b = 0;
             b < ouputdata.datafilevariables[a].doublevectorvalues.size();
             b++) {
          outputline =
              outputline + " " +
              std::to_string(
                  ouputdata.datafilevariables[a].doublevectorvalues[b]);
        }
      }
    }
...
void pessum::luxreader::OuputRawFileData(LuxFileType filetype,
                                         std::string filedirectory,
