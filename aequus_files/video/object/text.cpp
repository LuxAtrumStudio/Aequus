...
void aequus::video::Text::RenderText(std::string str, FontRenderMode mode) {
  ...
  if (mode == SHADEDTEXT) {
    ...
    textsurface =
        TTF_RenderText_Shaded(ttffont, text.c_str(), sdlcolor, sdlcolorbg);
  }
  if (mode == BLENDTEXT) {
...
void aequus::video::Text::CreateText(std::string str, int point,
                                     FontWeight weight, bool italic,
...
void aequus::video::Text::CloseFont() { TTF_CloseFont(ttffont); }
